        ...
        try {
            ...
            for (String authorData : autorsData) {
                ...
                for (Person p : personas.getPersonas()) {
                    if (p instanceof Author author && author.getId() == authorId) {
                        autoresFinales.add(author);
                        break;
                    }
                }
            }
            ...
            if (nroCopias.isBlank()) {
                return new Response("Copies must not be empty", Status.BAD_REQUEST);
            }
            ...
            if (nroPaginas.isBlank()) {
                return new Response("Pages must not be empty", Status.BAD_REQUEST);
            }
            ...
            PrintedBook libroTemp = new PrintedBook(
                    title, autoresFinales, isbn, genre, format, value, editorial, paginas, copias
            );

            libros.add(libroTemp);

            return new Response("PrintedBook created successfully", Status.CREATED);
