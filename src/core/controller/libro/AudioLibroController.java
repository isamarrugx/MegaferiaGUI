...
public class AudioLibroController {
    ...
    public static Response addAudioLibro(String title, String autorData, String isbn, String genre, String format, String valuetxt, String editorialData, String durationtxt, String narradorData) {
        ...
        try {
            ...
            if (narradorData.isBlank()) {
                return new Response("Narrator must not be empty", Status.BAD_REQUEST);
            }

            long narratorId;
            try {
                narratorId = Long.parseLong(narradorData.split(" - ")[0]);
            } catch (NumberFormatException e) {
            ...
            for (Person p : personas.getPersonas()) {
                if (p instanceof Narrator n && n.getId() == narratorId) {
                    narradorA = n;
                    break;
                }
            }
            ...
            if (narradorA == null) {
                return new Response("Narrator not found", Status.BAD_REQUEST);
            }

            AudioBook libroTemp = new AudioBook(
                    title, autoresFinales, isbn, genre, format, value, editorial, duracion, narradorA
            );
